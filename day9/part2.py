...
class Memory(dict):
    ...
    def read(self, address):
        return self.setdefault(address, 0)

    def read_vector(self, start_address, to_address):
        values = []
        for i in range(start_address, to_address):
...
class Processor:
    ...
    def run(self, input_stream, output_stream):
        ...
        while self._working:
            modes_and_opcode = str(self.memory.read(self._instruction_pointer))
            # "1102" -> "02"; "02" -> 2
            ...
            parameters = self.memory.read_vector(
    ...
    def _get_value(self, x):
        ...
        if mode == self.BY_REFERENCE:
            return self.memory.read(x[1])
        elif mode == self.BY_VALUE:
            return x[1]
        elif mode == self.BY_RELATIVE_REFERENCE:
            return self.memory.read(x[1] + self._relative_base_offset)

    def _write_value(self, x, z):
        if z[0] not in (self.BY_REFERENCE, self.BY_RELATIVE_REFERENCE):
            raise Exception('Wrong address mode')

        if z[0] == self.BY_REFERENCE:
            self.memory.write(z[1], x)
        elif z[0] == self.BY_RELATIVE_REFERENCE:
            self.memory.write(z[1] + self._relative_base_offset, x)
    ...
    def adjust_relative_base(self, x):
        x_v = self._get_value(x)
        self._relative_base_offset += x_v


def main():
    program = list(map(int, input_program.split(',')))

    memory = Memory({k: v for k, v in enumerate(program)})
    processor = Processor(memory)

    input_stream = [2]
    output_stream = []

    processor.run(input_stream, output_stream)

    print('Output: {}'.format(output_stream))
