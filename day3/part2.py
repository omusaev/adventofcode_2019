...
def main():
    ...
    for name, moves in known_wires.items():
        operations = {
            'R': ('x', operator.add),
            'L': ('x', operator.sub),
            'U': ('y', operator.add),
        ...
        for move in moves:
            ...
            coord, func = operations[direction]

            for i in range(0, length):
                coordinates[coord] = func(coordinates[coord], 1)
                steps += 1

                grid.setdefault(
                    (coordinates['x'], coordinates['y']), []).append((name, steps))

    intersections = []

    for point, wires_and_steps in grid.items():
        intersection_wires = set([wire for wire, steps in wires_and_steps])

        # all wires presented
        if intersection_wires == set(known_wires.keys()):
            intersections.append((point, wires_and_steps))

    print('Intersections: {}'.format(intersections))

    def _combined_steps(intersection):
        point, wires_and_steps = intersection
        steps_total = 0
        for name in known_wires.keys():
            # look for the first entry
            for wire, steps in wires_and_steps:
                if wire == name:
                    steps_total += steps

        return steps_total

    closest = min(intersections, key=_combined_steps)
    minimal_combined_steps = _combined_steps(closest)

    print('Minimal combined steps intersection: {}'.format(closest))
    print('Minimal combined steps: {}'.format(minimal_combined_steps))
